...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    if (cycle_mask & bit(idx)) {
      ...
      #else
        if ( bit_istrue(settings.homing_dir_mask,bit(idx)) ) {
          set_axis_position = lroundf((settings.max_travel[idx]+settings.homing_pulloff)*settings.steps_per_unit[idx]);
        } else {
          set_axis_position = lroundf(-settings.homing_pulloff*settings.steps_per_unit[idx]);
        }
      #endif
